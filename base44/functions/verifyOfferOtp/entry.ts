...
async function buildAcceptedPdf(offer, verificationDetails) {
  ...
  const acceptedStr = 'ΑΠΟΔΕΚΤΗ ΠΡΟΣΦΟΡΑ – ELECTRONIC ACCEPTANCE CONFIRMED';
  page1.drawText(acceptedStr, { x: 25, y: y - 15, size: 8, font: boldFont, color: white });
  y -= 30;
  ...
  const note = 'Αυτό το έγγραφο αποτελεί ηλεκτρονική απόδειξη αποδοχής μέσω OTP επαλήθευσης email.';
...
Deno.serve(async (req) => {
  ...
  const { offer_id, otp_code } = body;
  if (!offer_id || !otp_code) return Response.json({ error: 'Missing offer_id or otp_code' }, { status: 400 });
  ...
  // Check OTP exists
  if (!offer.otp_hash || !offer.otp_expires_at) {
    return Response.json({ error: 'Δεν υπάρχει ενεργό OTP. Παρακαλώ ζητήστε νέο κωδικό.' }, { status: 400 });
  }

  // Check expiry
  if (new Date(offer.otp_expires_at) < new Date()) {
    return Response.json({ error: 'Ο κωδικός έχει λήξει. Παρακαλώ ζητήστε νέο.' }, { status: 400 });
  }

  // Check max attempts
  const attempts = (offer.otp_attempts || 0) + 1;
  if (attempts > 5) {
    return Response.json({ error: 'Υπερβήκατε τον μέγιστο αριθμό προσπαθειών. Παρακαλώ ζητήστε νέο κωδικό.' }, { status: 429 });
  }

  // Verify hash
  const inputHash = await sha256(String(otp_code).trim());
  let auditLog = [];
  try { auditLog = JSON.parse(offer.audit_log || '[]'); } catch {}

  if (inputHash !== offer.otp_hash) {
    // Wrong code
    await base44.asServiceRole.entities.ResellerOffer.update(offer_id, {
      ...
      audit_log: JSON.stringify([...auditLog, {
        ...
        timestamp: now,
        actor: 'customer',
        details: { ip, user_agent: userAgent, attempts }
      }])
    });
    const remaining = 5 - attempts;
    return Response.json({ error: `Λάθος κωδικός. Απομένουν ${remaining} προσπάθειες.` }, { status: 400 });
  }

  // OTP verified — mark accepted
  ...
  auditLog.push({ action: 'otp_verified', timestamp: now, actor: 'customer', details: { ip, user_agent: userAgent } });
  auditLog.push({ action: 'accepted', timestamp: now, actor: 'customer', details: { ip } });
