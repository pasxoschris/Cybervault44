...
export default function ResolutionStatusManager() {
  ...
  const load = () => {
    setLoading(true);
    base44.entities.ResolutionStatus.list('display_order', 50).then(setItems).finally(() => setLoading(false));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.mode === 'add') await base44.entities.ResolutionStatus.create(form);
      else await base44.entities.ResolutionStatus.update(modal.item.id, form);
      setModal(null); load();
  ...
  const handleDelete = async (item) => {
    ...
    await base44.entities.ResolutionStatus.delete(item.id);
    load();
  };

  const handleToggle = async (item) => {
    await base44.entities.ResolutionStatus.update(item.id, { is_active: !item.is_active });
    load();
  };

  const handleReorder = async (reordered) => {
    setItems(reordered);
    await base44.entities.ResolutionStatus.bulkUpdate(reordered.map((item, i) => ({ id: item.id, display_order: i })));
  };
  ...
  return (
    <>
      ...
      {modal && (
        <AdminModal title={modal.mode === 'add' ? 'Νέο Resolution Status' : 'Επεξεργασία Resolution Status'}
          ...
          <div>
            ...
            <input className="cyber-input" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="π.χ. Resolved" />
          </div>
        </AdminModal>
      )}
    </>
